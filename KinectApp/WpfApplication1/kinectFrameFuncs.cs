...
{
    class KinectFrameHandler : IKinectFrameHandler
    {
        #region Variable
        Model model;
        ...
        const int ROUND = 1;
        System.Windows.Controls.Image screen;
        ...
        Skeleton curSkeleton = null, preSkeleton = null;//skeletons

        void IKinectFrameHandler.skeletonFrameReady(object sender, SkeletonFrameReadyEventArgs e)
        {
        ...
        public double getHeight(Joint j)
        {
            Vector joint = new Vector(j);
            double ang = -model.getVerticalAngle() * Math.PI / 180 + model.getKinectAngle();//to radians
            double tmp = joint.Y * Math.Cos(ang) - joint.Z * Math.Sin(ang);
            return model.getKinectHeight() + tmp;
        }

        public Skeleton clone(Skeleton skOrigin)
        {
        ...
        void IKinectFrameHandler.depthFrameReady(object sender, DepthImageFrameReadyEventArgs e)
        {
        ...
        public int createColor(byte alpha, byte red, byte green, byte blue)
        {
            return (alpha << 24) | (roundBy(red, ROUND) << 16) | (roundBy(green, ROUND) << 8) | roundBy(blue, ROUND);
        }
        public int roundBy(int num, int round)
        {
            num /= round;
            return num * round;
        }
    ...
    class KinectAngleHandler: IKinectAngleHandler
    {
    ...
    enum situation {standing, sitting, lying, moving, notMoving, rightHand, leftHand};
    ...
    enum PositionType { standing, sitting, lying, rightHand, leftHand, moves, notMoves };
    class AlertHandler : IAlertHandler
    {
